...
object GoogleVsApple:
  ...
  lazy val googleTweets: TweetSet = TweetReader.allTweets.filter(tweet => google.map(name => tweet.text.contains(name)).exists(bool => bool))
  lazy val appleTweets: TweetSet = TweetReader.allTweets.filter(tweet => apple.map(name => tweet.text.contains(name)).exists(bool => bool))
